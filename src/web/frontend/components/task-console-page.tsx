...
function TaskConsoleContent() {
  ...
  async function handleStartJob() {
    ...
    try {
      ...
      await refreshHistoryJobs();
      router.push(`/jobs/${jobId}` as Parameters<typeof router.push>[0]);
    } catch (error) {
      setRunNotice({
        tone: "error",
                ...
                <tr key={row.id}>
                  ...
                  <td>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <ActionButton
                        variant="secondary"
                        onClick={() => router.push(`/jobs/${row.id}` as Parameters<typeof router.push>[0])}
                      >
                        View Job
                      </ActionButton>
                    </div>
